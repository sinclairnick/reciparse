...
export const AdamLiaw = createParser({
  ...
  parse: (html: string) => {
    ...
    const idxOfIngredients = childrenText.findIndex(
      (x) => x.toLowerCase() === "ingredients"
    );
    const idxOfMethod = childrenText.findIndex(
      (x) => x.toLowerCase() === "method"
    );

    const _ingredients = childrenText.slice(idxOfIngredients + 1, idxOfMethod);
    const _steps = childrenText.slice(idxOfMethod + 1);

    const ingredients: Ingredient[] = extractIngredientsFromRaw(_ingredients);
